...
main() {
  ...
  if [[ "${target_platform}" == "win-"* ]]; then
    ...
    export CFLAGS="$CFLAGS --target=x86_64-apple-darwin13.4.0 -march=x86-64 -mmacosx-version-min=10.13"
    export LDFLAGS="$LDFLAGS --target=x86_64-apple-darwin13.4.0 -march=x86-64 -mmacosx-version-min=10.13"
    export CC_FOR_BUILD="$CC --target=x86_64-apple-darwin13.4.0 -march=x86-64 -mmacosx-version-min=10.13"
    ...
    # Force architecture and deployment target in GHC settings with consistent target
    sed -i -E "s#(C compiler flags\", \")#\1--target=x86_64-apple-darwin13.4.0 -march=x86-64 -mmacosx-version-min=10.13 #" "${settings_file}"
    sed -i -E "s#(C compiler link flags\", \")#\1--target=x86_64-apple-darwin13.4.0 -march=x86-64 -mmacosx-version-min=10.13 -Wl,${SDKROOT}/usr/lib/libiconv.2.tbd #" "${settings_file}"
...
EOF
  ...
  if [[ "${target_platform}" == "osx-"* ]]; then
    cat >> cabal.release.constraints.project << EOF
package *
  ghc-options: -optc--target=x86_64-apple-darwin13.4.0 -optc-march=x86-64 -optc-mmacosx-version-min=10.13 -optl--target=x86_64-apple-darwin13.4.0 -optl-march=x86-64 -optl-mmacosx-version-min=10.13
EOF
  fi
